...
def test(example, model_names):
    ...
    for initial_regul, initrecon in methods:
        ...
        X, Y, Z = get_sample(index, example, initrecon, initial_regul, "validation", config)
        ...
        if not plotted:
            ...
            plt.axis("off")
            plt.savefig(f"results/figures/0000-{example}-data_ground_truth.pdf", bbox_inches="tight")
            ...
            plotted = True
            np.save(f"results/figures/0000-{example}_ground_truth_image", X.cpu().numpy().squeeze().astype('float64'))

        radon_init_recon = radon_full.forward(Y).cpu().numpy().squeeze().astype('float64')
        plt.figure()
        plt.imshow(radon_init_recon, cmap="gray")
        plt.axis("off")
        plt.savefig(f"results/figures/0000-{example}-data_{initial_regul}.pdf", bbox_inches="tight")

        plt.figure()
        plt.imshow(abs(radon_init_recon-radon_gt), cmap="gray")
        plt.axis("off")
        plt.savefig(f"results/figures/0000-{example}-data_{initial_regul}_diff.pdf", bbox_inches="tight")
        ...
        np.save(f"results/figures/0000-{example}_{initial_regul}_image", Y.cpu().numpy().squeeze().astype('float64'))
        np.save(f"results/figures/0000-{example}_{initial_regul}_data", radon_init_recon)
    ...
    for model_name in model_names:
        ...
        radon_recon = radon_full.forward(model_output).cpu().detach().numpy().squeeze().astype('float64')

        plt.figure()
        plt.imshow(radon_recon, cmap="gray")
        plt.axis("off")
        plt.savefig(f"results/figures/0000-{example}-data_{model_name}.pdf", bbox_inches="tight")
        
        plt.figure()
        plt.imshow(abs(radon_recon-radon_gt), cmap="gray")
        plt.axis("off")
        plt.savefig(f"results/figures/0000-{example}-data_{model_name}_diff.pdf", bbox_inches="tight")
        
        errors_data1[method_latex_mapping[model_name]] = mse(
            radon_full.forward(model_output).cpu().numpy().squeeze().astype('float64')*chi,
            extension_with_zero(Z, config).cpu().numpy().squeeze().astype('float64')
        )
        ...
        errors_data3[method_latex_mapping[model_name]] = mse(
            radon_full.forward(model_output).cpu().numpy().squeeze().astype('float64')*(1-chi),
            radon_full.forward(X).cpu().numpy().squeeze().astype('float64')*(1-chi)
        )

        np.save(f"results/figures/0000-{example}_{model_name}_image", model_output.cpu().numpy().squeeze().astype('float64'))
    ...
    errors = {}
    errors["$\\radon_\\Omega (\\signal_{{\\rm REC}}) - \\data^\\delta$"] = errors_data1 # [val for _, val in errors_data1.items()]
    errors["$\\radon_\\Omega (\\signal_{{\\rm REC}}) - \\radon_\\Omega (\\signal^\\star)$"] = errors_data2 # [val for _, val in errors_data3.items()]
    errors["$\\radon_{{\\Omega^c}} (\\signal_{{\\rm REC}}) - \\radon_{{\\Omega^c}} (\\signal^\\star)$"] = errors_data3 # [val for _, val in errors_data3.items()]
