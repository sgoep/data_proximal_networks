...
def test_other_function_class(example, model_names, zoom=False, colorbar=False):
    ...
    (X, data_limited, fbp_ellipse,
     tv_ellipse, ell1_ellipse, landweber_ellipse) = create_ellipse_example(example)
    # (X, data_limited, fbp_ellipse,
    #  tv_ellipse, ell1_ellipse, landweber_ellipse) = create_lotus_example()

    visualization_with_zoom(
        example,
        X.cpu().numpy().squeeze().astype('float64'), False, False,
    ...
    if True:
        ...
        visualization_with_zoom(
            example,
            fbp_ellipse.cpu().numpy().squeeze().astype('float64'), zoom, colorbar,
        ...
        visualization_with_zoom(
            example,
            landweber_ellipse.cpu().numpy().squeeze().astype('float64'), zoom, colorbar,
        ...
        print("tv.")
        # Process TV
        errors["tv"] = calc_errors(tv_ellipse.cpu().numpy().squeeze().astype('float64'), X.cpu().numpy().squeeze().astype('float64'))
        print_errors(tv_ellipse.cpu().numpy().squeeze().astype('float64'), X.cpu().numpy().squeeze().astype('float64'))

        # Visualize the result
        visualization_with_zoom(
            example,
            tv_ellipse.cpu().numpy().squeeze().astype('float64'), zoom, colorbar,
        ...
        # Process ELL1
        print("ell1.")
        errors["ell1"] = calc_errors(ell1_ellipse.cpu().numpy().squeeze().astype('float64'), X.cpu().numpy().squeeze().astype('float64'))
        print_errors(ell1_ellipse.cpu().numpy().squeeze().astype('float64'), X.cpu().numpy().squeeze().astype('float64'))

        # Visualize the result
        visualization_with_zoom(
            example,
            ell1_ellipse.cpu().numpy().squeeze().astype('float64'), zoom, colorbar,
