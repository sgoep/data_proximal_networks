...
'''
The following code is taken from https://github.com/drgHannah/Radon-Transformation
'''
...
class radon(torch.nn.Module):
    ...
    def __init__(self, angles, n_angles=1000, image_size=400, device="cuda"):
        super(radon, self).__init__()
        self.n_angles=n_angles
        # get angles 
        # thetas = torch.linspace(0, np.pi-(np.pi/n_angles), n_angles)[:,None,None].to(device)
        # thetas = torch.linspace(-ran_angles, ran_angles*(1-1/n_angles), n_angles)[:,None,None] 
        thetas = angles[:,None,None]
        ...
        self.rotated = torch.nn.functional.affine_grid(rotations, torch.Size([n_angles, 1, image_size, image_size]), align_corners=True).reshape(1,-1,image_size,2).to(device)
...
class fbp(torch.nn.Module):
    ...
    Args:
        ...
        device: (str): device can be either "cuda" or "cpu" (default: cuda)
    '''
    def __init__(self, angles, n_angles=1000, image_size=400, circle = False, filtered=True, device="cuda"):
        super().__init__()
...
def get_operators(angles, n_angles=380, image_size=400, circle = False, filtered=True, device='cuda'):
    ...
    # radon_op = radon(ran_angles=ran_angles, n_angles=n_angles, image_size=image_size, device=device)
    # fbp_op = fbp(ran_angles=ran_angles, n_angles=n_angles, image_size=image_size, circle=circle, device=device)
    
    radon_op = radon(angles=angles, n_angles=n_angles, image_size=image_size, device=device)
    fbp_op = fbp(angles=angles, n_angles=n_angles, image_size=image_size, circle=circle, filtered=filtered, device=device)
