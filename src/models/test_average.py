...
def test(example, model_names):
    ...
    errors_data = {
        name: {
            "$\\radon_\\Omega (\\signal_{{\\rm REC}}) - \\data^\\delta$": None,
            "$\\radon_\\Omega (\\signal_{{\\rm REC}}) - \\radon_\\Omega (\\signal^\\star)$": None,
            "$\\radon_{{\\Omega^c}} (\\signal_{{\\rm REC}}) - \\radon_{{\\Omega^c}} (\\signal^\\star)$": None,
        }
        for name in model_names
    }
    errors_data["landweber"] = {
        "$\\radon_\\Omega (\\signal_{{\\rm REC}}) - \\data^\\delta$": None,
        "$\\radon_\\Omega (\\signal_{{\\rm REC}}) - \\radon_\\Omega (\\signal^\\star)$": None,
        "$\\radon_{{\\Omega^c}} (\\signal_{{\\rm REC}}) - \\radon_{{\\Omega^c}} (\\signal^\\star)$": None,
    }
    ...
    for initial_regul in ["landweber", "tv", "ell1"]:
        ...
        TestDataGen = torch.utils.data.DataLoader(
            model_test_set, batch_size=1, shuffle=False, num_workers=2
        )
        ...
        for X, Y, Z in TestDataGen:
            ...
            radon_X = (
                radon_full.forward(torch.Tensor(X).float().cuda())
                .cpu()
                .numpy()
                .squeeze()
            )
            radon_Y = (
                radon_full.forward(torch.Tensor(Y).float().cuda())
                .cpu()
                .numpy()
                .squeeze()
            )
            ...
            mse_list_data3.append(
                mse(radon_Y.astype("float64") * (1 - chi), radon_X * (1 - chi))
            )
    ...
    for model_name in model_names:
        ...
        for X, Y, Z in TestDataGen:
            ...
            mse_list_data2.append(
                mse(radon_output.astype("float64") * chi, radon_X * chi)
            )
            mse_list_data3.append(
                mse(radon_output.astype("float64") * (1 - chi), radon_X * (1 - chi))
            )
            ...
            psnr_list.append(
                psnr(
                    X, model_output, data_range=model_output.max() - model_output.min()
                )
            )
            ssim_list.append(
                ssim(
